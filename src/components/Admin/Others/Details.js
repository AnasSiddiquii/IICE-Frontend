...
const Details = () => {
  ...
  const getDetail = async () => {
    let result = await fetch('https://new.iice.foundation/details')
  ...
  const remove = async (id) => {
    let result = await fetch(`https://new.iice.foundation/deletedetail/${id}`,{
      method:'delete'
    })
    result = await result.json()
    if(result){
  ...
  const search = async(e) => {
    ...
    if(key){
      let result = await fetch(`https://new.iice.foundation/searchdetail/${key}`)
      result = await result.json()
      if(result){
        setDetail(result)
      }
